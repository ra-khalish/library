...
class Book extends Component {
  ...
  toggle = buku =>
    this.setState({ modal: !this.state.modal, selectedBook: buku });

  // TODO: Isi fungsi untuk memanggil method PUT untuk fitur merubah status peminjaman
  EditBook = buku => {
    axios
      .put(
        "https://library2020-api-rafi.herokuapp.com/library/" + buku._id,
        buku
      )
      .then(res => {
      ...
      <div>
        ...
        >
          <CardTitle>
            ...
            <Badge
              ...
              color={isDipinjam ? "danger" : "success"}
            >
        ...
        <Modal isOpen={this.state.modal} toggle={this.toggle}>
          <ModalHeader toggle={this.toggle}>
            {this.state.selectedBook.judulBuku}
          </ModalHeader>
          <ModalBody>
            Apakah Anda yakin untuk {isDipinjam ? "mengembalikan" : "meminjam"}{" "}
            buku ini?
          </ModalBody>
          <ModalFooter>
            <Button color="primary" onClick={this.onSubmit}>
              {isDipinjam ? "Kembalikan Buku" : "Pinjam Buku"}
            </Button>{" "}
